...
user_movie_matrix = subset_data.pivot_table(
    index='userId',
    columns='title',
    values='rating'
)
...
cosine_sim = cosine_similarity(user_movie_filled.T)  # transpose = movies x users
...
cosine_sim_df.to_pickle(os.path.join(model_dir, "cosine_sim.pkl"))
